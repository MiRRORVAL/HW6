...
class EmloyeeListViewController: UITableViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let indexPath = tableView.indexPathForSelectedRow {
            let detailVS = segue.destination as! DetailViewController
            detailVS.personeNameAndSername = (arrayOfEmployees[indexPath.row].name + " " + arrayOfEmployees[indexPath.row].surname)
            detailVS.phoneNumber = arrayOfEmployees[indexPath.row].phoneNumber
            detailVS.emailAdres = arrayOfEmployees[indexPath.row].email
        }
    }
}
